...
void gui()
{
    ...
    display.register_button_handler(
        [&](const win::Button button, const bool press)
        {
    ...
    display.register_resize_handler(
        [&renderer, &cpanel, &cpanel_box, &lpanel, &lpanel_box](int width, int height)
        {
    ...
    while (!quit)
    {
        ...
        if (std::chrono::duration<float, std::milli>(std::chrono::high_resolution_clock::now() - last_interaction).count() > 10'000)
