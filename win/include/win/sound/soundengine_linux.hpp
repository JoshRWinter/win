...
public:
	...
	virtual std::uint32_t play(const char*, int, float, bool, int) = 0;
	virtual std::uint32_t play(const char*, int, float, float, float, bool, int) = 0;
...
public:
	...
	std::uint32_t play(const char *path, int residency_priority, float compression_priority, bool looping = false, int seek = 0) { return inner->play(path, residency_priority, compression_priority, looping, seek); }
	std::uint32_t play(const char *path, int residency_priority, float compression_priority, float left, float right, bool looping = false, int seek = 0) { return inner->play(path, residency_priority, compression_priority, left, right, looping, seek); }
