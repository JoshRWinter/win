...
public:
    ...
    MappedRingBufferRangeIterator<T> operator++()
    {
        ++position;
        return MappedRingBufferRangeIterator<T>(parent, position);
    }

    MappedRingBufferRangeIterator<T> operator++(int)
    {
        ++position;
        return MappedRingBufferRangeIterator<T>(parent, position - 1);
    }
    ...
    T &operator*()
    {
        ...
        if (position < 0 || position >= parent->range_length)
            win::bug("MappedRingBufferRangeIterator: Attempt to access invalid range index " +
                     std::to_string(position) +
                     " when range length is " +
                     std::to_string(parent->range_length) +
                     ".");
#endif
...
public:
    ...
    void write(const int offset, const T *const src, const int len)
    {
        ...
        if (len + offset > range_length)
            win::bug("MappedRingBufferRange: write too long. Requested write length was " +
                     std::to_string(len) +
                     " (from offset " +
                     std::to_string(offset) +
                     ") while the range length is only " +
                     std::to_string(range_length) +
                     ".");
#endif
...
public:
    ...
    MappedRingBufferRange<T> reserve(int len)
    {
        ...
        if (len > buffer_length)
            win::bug("MappedRingBuffer: reservation too long. Requested length was " +
                     std::to_string(len) +
                     " while the total buffer length is only " +
