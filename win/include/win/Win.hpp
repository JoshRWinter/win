...
#define WIN_BENCHMARK(name, x, thresholdms)                                                                                                                    \
    ...
    x;                                                                                                                                                         \
    const float WIN_TOKEN_APPEND(win_benchmark_elapsed_ms, __LINE__) =                                                                                         \
        std::chrono::duration<float, std::milli>(std::chrono::high_resolution_clock::now() - WIN_TOKEN_APPEND(win_benchmark_start, __LINE__)).count();         \
    if (WIN_TOKEN_APPEND(win_benchmark_elapsed_ms, __LINE__) >= thresholdms)                                                                                   \
    fprintf(stderr, "%s: %.4fms\n", name, WIN_TOKEN_APPEND(win_benchmark_elapsed_ms, __LINE__))
...
#define WIN_INTERVAL(name, thresholdms)                                                                                                                        \
    ...
    const auto WIN_TOKEN_APPEND(win_interval_elapsed_ms, __LINE__) =                                                                                           \
        std::chrono::duration<float, std::milli>(WIN_TOKEN_APPEND(win_interval_current_time, __LINE__) - WIN_TOKEN_APPEND(win_interval_last_time, __LINE__))   \
            .count();                                                                                                                                          \
    if (WIN_TOKEN_APPEND(win_interval_elapsed_ms, __LINE__) >= thresholdms)                                                                                    \
