	...
	, mixer(roll)
{
	...
	format.nAvgBytesPerSec = 44'100 * 4;
...
bool SoundEngineWindowsDirectSound::write()
{
	...
	if ((play_position < write_position && write_position < safe_write_position) ||
	...
	const DWORD available_to_write_bytes =
		write_position < play_position ? play_position - write_position : ((buffer_len_bytes - write_position) + play_position);
